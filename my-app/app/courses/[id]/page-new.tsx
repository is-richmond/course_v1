...
export default function CoursePageNew({ params: paramsPromise }: PageProps) {
  ...
  if (!course || !courseId) {
    return isLoading ? <div>Загрузка...</div> : <notFound />;
  }
  ...
  const selectedModule = course.syllabus.find((m) => m.id === selectedModuleId);
